...
describe('App API Endpoints', () => {
  ...
  const seedListTwoId = 2
  const seedListTwoItemId = 2
  ...
  describe('POST /api/todo-list/:listId/item', () => {
    it('should add a new todo item to a specific list', async () => {
      const postRes = await request(app).post(`/api/todo-list/${seedListOneId}/item`).send({ itemTitle: 'New Item' })
      expect(postRes.status).toBe(200)
      const getRes = await request(app).get(`/api/todo-list/${seedListOneId}/item/${postRes.body.id}`)
      expect(getRes.body).toMatchObject({
        itemTitle: 'New Item',
        list: { id: seedListOneId, title: 'First List' }
      })
    })
    ...
    it('should mark the list as incomplete after adding a new item', async () => {
      ...
      const itemOne = await request(app).post(`/api/todo-list/${seedListTwoId}/item`).send({
        itemTitle: 'Item 1',
        completed: true
      })
      const itemTwo = await request(app).post(`/api/todo-list/${seedListTwoId}/item`).send({
        itemTitle: 'Item 2',
        completed: true
      })
      ...
      const listBefore = await request(app).get(`/api/todo-list/${seedListTwoId}`)
      ...
      const postRes = await request(app).post(`/api/todo-list/${seedListTwoId}/item`).send({ itemTitle: 'New Incomplete Item' })
      expect(postRes.status).toBe(200)

      // Step 3: Check if the list is now marked as incomplete
      const listAfter = await request(app).get(`/api/todo-list/${seedListTwoId}`)
  ...
  describe('PATCH /api/todo-list/:listId/item/:itemId', () => {
    ...
    it('should update a specific todo item in a list', async () => {
      const postRes = await createItem(seedListOneId, { itemTitle: 'New Item', completed: true })
      expect(postRes.status).toBe(200)

      const patchRes = await patchItem(seedListOneId, postRes.body.id, { itemTitle: 'Updated Item' })
      expect(patchRes.status).toBe(200)

      expect(patchRes.body).toMatchObject({ itemTitle: 'Updated Item', completed: true })

      const getRes = await request(app).get(`/api/todo-list/${seedListOneId}/item/${patchRes.body.id}`)
    ...
    it('should update list completed status to true when all items are completed', async () => {
      const res = await getList(seedListTwoId)
      expect(res.status).toBe(200)
      expect(res.body).toEqual({
        id: seedListTwoId,
        title: 'Second List',
      ...
      const postRes = await createItem(seedListTwoId, { itemTitle: 'New Item', completed: true })
      expect(postRes.status).toBe(200)

      const patchRes = await patchItem(seedListTwoId, seedListTwoItemId, { itemTitle: 'First todo updated', completed: true })
      expect(patchRes.status).toBe(200)
      expect(patchRes.body).toMatchObject({ completed: true })

      const patchResFirstItem = await patchItem(seedListTwoId, seedListTwoItemId, { completed: true })
      expect(patchResFirstItem.status).toBe(200)

      const resAfter = await getList(seedListTwoId)
    ...
    it('should update list completed status to False when one or more items are incomplete', async () => {
      ...
      const patchRes = await patchItem(seedListTwoId, res.body.items[0].id, { itemTitle: 'Incomplete', completed: false })
      expect(patchRes.status).toBe(200)
    ...
    it('should keep list completed status as False when not all items are completed', async () => {
      const itemOneRes = await createItem(seedListOneId, { itemTitle: 'Incomplete Item', completed: false })
      expect(itemOneRes.status).toBe(200)

      const itemTwoRes = await createItem(seedListOneId, { itemTitle: 'Completed Item', completed: true })
  ...
  describe('DELETE /api/todo-list/:listId/item/:itemId', () => {
    ...
    it('should update list completed status to true if all remaining items are completed after deletion', async () => {
      ...
      const itemOneRes = await request(app)
        .post(`/api/todo-list/${seedListTwoId}/item`)
      ...
      const deleteRes = await request(app).delete(`/api/todo-list/${seedListTwoId}/item/${itemOneRes.body.id}`)
      ...
      const listRes = await request(app).get(`/api/todo-list/${seedListTwoId}`)
